---
title: "Getting started with datacore"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting started with datacore}
...
`datacore` is the official R client for [DataCore](https://datacore.vn), the
Vietnamese financial and alternative data platform. It is built to feel like a
native tidyverse package: every endpoint returns a tibble, dates are real
`Date` objects, and the client respects rate limits and retries transparently.
...
Get a key from <https://datacore.vn> and set it as an environment variable.
The recommended place is `~/.Renviron`:

```
DATACORE_API_KEY=dc_live_xxxxxxxxxxxxxxxx
```

Restart your R session and you're ready. You can also pass the key explicitly
for one-off scripts:
...
- The [DataCore cookbook](https://github.com/DataCore-VietNam/datacore-cookbook)
  for end-to-end examples (factor models, event studies, Tidy Finance VN)
